...
const GROUPS = {
    'VPN': { 
        name: 'VPN',
        description: 'Accès VPN distant',
        type: 'Sécurité'
    },
    'Sortants_responsables': { 
    ...
    >
        <Box sx={{ flex: 1, pr: 2 }}>
            ...
            <Typography variant="caption" color="text.secondary">
                {member.SamAccountName || member.sam}
            </Typography>
        </Box>
...
const TableHeader = memo(() => (
    <Box sx={{ 
        ...
        <Box sx={{ flex: 1 }}>
            <Typography variant="subtitle2" fontWeight={600}>Membre</Typography>
        </Box>
...
const AdGroupsPage = () => {
    ...
    const loadGroupMembers = async (force = false) => {
        const loadingState = force ? setIsRefreshing : setIsLoading;
        loadingState(true);

        try {
            const cacheKey = `adGroups:${selectedGroup}`;
            const { data: membersData } = await fetchWithCache(
                cacheKey,
                () => window.electronAPI.getAdGroupMembers(selectedGroup),
                { force }
            );
    ...
    const handleRemoveUser = async (username, groupName) => {
        ...
        try {
            const result = await window.electronAPI.removeUserFromGroup({ 
                username, 
                groupName 
            });
    ...
    const handleAddUser = async (username) => {
        try {
            const result = await window.electronAPI.addUserToGroup({ 
                username, 
    ...
    const searchAdUsers = async (term) => {
        ...
        try {
            const users = await window.electronAPI.searchAdUsers(term);
            setAvailableUsers(users || []);
        } catch (error) {
            showNotification('error', `Erreur recherche: ${error.message}`);
            setAvailableUsers([]);
        } finally {
    ...
    return (
        <Box sx={{ p: 2, height: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column' }}>
            ...
            <Paper elevation={3} sx={{ p: 2, mb: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        <Tooltip title="Actualiser">
                            <IconButton onClick={() => loadGroupMembers(true)} disabled={isRefreshing}>
                ...
                <Box sx={{ mt: 2, p: 1.5, backgroundColor: 'info.lighter', borderRadius: 1, display: 'flex', gap: 1 }}>
            ...
            <Dialog open={addUserDialogOpen} onClose={() => setAddUserDialogOpen(false)} maxWidth="sm" fullWidth>
                            ...
                            <List>
                                {availableUsers.map((user, index) => {
                                    const alreadyMember = members.some(m => 
                                        (m.SamAccountName || m.sam) === user.SamAccountName
                                    );
                                    ...
                                    return (
                                        <React.Fragment key={user.SamAccountName}>
                                            <ListItem
                                                button
                                                onClick={() => !alreadyMember && handleAddUser(user.SamAccountName)}
                                            ...
                                            >
                                                ...
                                                />
                                                {alreadyMember && (
                                                    <Chip label="Déjà membre" size="small" color="success" />
                                                )}
                                            </ListItem>
                ...
                <DialogActions>
                    <Button onClick={() => setAddUserDialogOpen(false)}>Fermer</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
