...
async function connectWithRetry(retryCount = 0) {
    ...
    try {
        // Test rapide d'accès au répertoire (timeout court)
        ...
        // Test d'accès en écriture avec timeout
        const testFile = path.join(dir, '.write-test');
        try {
            fs.writeFileSync(testFile, 'test');
            fs.unlinkSync(testFile);
        } catch (accessError) {
            throw new Error(`Serveur réseau inaccessible`);
        }
        
        // Connexion à la base de données réseau
        ...
        if (retryCount < MAX_RETRIES && isNetworkPath) {
            console.log(`⚠️ Tentative ${connectionAttempts}/${MAX_RETRIES} échouée...`);
            await sleep(RETRY_DELAY_MS);
            return await connectWithRetry(retryCount + 1);
        } else {
            // Après échec, basculer en mode OFFLINE
            console.log('\n⚠️  SERVEUR RÉSEAU INACCESSIBLE - BASCULEMENT EN MODE OFFLINE');
            console.log(`   Chemin réseau: ${dbPath}`);
            console.log(`   → Utilisation base locale: ${LOCAL_DB_PATH}\n`);
...
function connectToLocalDatabase() {
    try {
        // Créer le répertoire data local si nécessaire
        ...
        console.log(`✅ Base de données SQLite connectée (OFFLINE MODE) : ${LOCAL_DB_PATH}`);
        console.log('💡 L\'app fonctionne en mode OFFLINE - Les données ne seront pas synchronisées avec le serveur réseau');
        return true;
    } catch (error) {
